...

{
    ...
    "title": "Post and Wait",
    "category": "section",
    "text": "To unlock the semaphore sem, call:post(sem)which increments by one the semaphore\'s value.  If the semaphore\'s value consequently becomes greater than zero, then another process or thread blocked in a wait call will be woken up and proceed to lock the semaphore.Locking the semaphore sem is done by:wait(sem)which decrements by one the semaphore sem.  If the semaphore\'s value is greater than zero, then the decrement proceeds, and the function returns, immediately.  If the semaphore currently has the value zero, then the call blocks until either it becomes possible to perform the decrement (i.e., the semaphore value rises above zero), or a signal handler interrupts the call (in which case an instance of InterruptException is thrown).  A SystemError may be thrown if an unexpected error occurs.To try locking the semaphore sem without blocking, do:trywait(sem) -> booleanThis call attempts to immediately decrement (lock) the semaphore returning true if successful.  If the decrement cannot be immediately performed, then false is returned.  If an interruption is received or if an unexpected error occurs, an exception is thrown (InterruptException or SystemError repectively).Finally, the call:timedwait(sem, secs)decrements (locks) the semaphore sem.  If the semaphore\'s value is greater than zero, then the decrement proceeds, and the function returns, immediately. If the semaphore currently has the value zero, then the call blocks until either it becomes possible to perform the decrement (i.e., the semaphore value rises above zero), or the limit of secs seconds expires (in which case an instance of TimeoutError is thrown), or a signal handler interrupts the call (in which case an instance of InterruptException is thrown)."
},
...

{
    ...
    "title": "IPC.post",
    "category": "method",
    "text": "post(sem)\n\nincrements (unlocks) the semaphore sem.  If the semaphore\'s value consequently becomes greater than zero, then another process or thread blocked in a wait call will be woken up and proceed to lock the semaphore.\n\nSee also: Semaphore, wait, timedwait,           trywait.\n\n\n\n\n\n"
},
